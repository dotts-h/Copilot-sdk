...
| 9 | The Telemetry **budget / month-to-date** rows still read the live (this-process) `Meter`, not the persisted ledger, so they reset on restart even though the trend view does not. | internal/web (`telemetryPartial`) | low | M | med | [ADR 0009](adr/0009-persisted-spend-history-append-only-ledger.md) | when "remaining this month" must survive a restart — reconcile the budget rows against `SpendStore` (and pair with item 3.2 per-session totals) |
