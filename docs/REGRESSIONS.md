...
| 3 | **Telemetry was thin** — no cache-write tokens, thinking tokens, message/tool counts, cache-hit %, session time, or USD in view. | `CacheWriteTokens` was unmapped from the SDK; no per-session counters; no statusline. | Map `CacheWriteTokens`; accumulate cache-write + reasoning as display-only totals (`Meter.ExtraTokens`); Server tracks session start / messages / tools; statusline renders the full split + cache-hit % + credits/USD. **(Update: cache-write is no longer display-only — it is priced at 1.25× input as of ADR-0034 / issue 0059; reasoning stays display-only as a subset of output.)** | unit: `internal/web` `TestUsageEmitsStatlineWithTokenBreakdown`, `TestToolStartCountsTowardStatline`; `internal/telemetry` token tests |
...
| 17 | The Runs **duration cell could print an impossible "60s" / "1m 60s"** for a sub-second-fractional span. Caught by the pre-merge code review of the V1 diff. | `humanDuration` rounded each unit *after* decomposing — `(d % time.Minute).Round(time.Second)` on a 59.6s span yields 60s, printed verbatim with no carry into the next unit (59.6s → "60s"; 1m59.6s → "1m 60s"). | Round the whole span to a clean resolution *up front* (to the second below an hour, the minute above), then decompose with plain integer division — a unit that rounds up to 60 carries naturally (59.6s → "1m"; 59m59.6s → "1h"). | unit: `internal/web` `TestHumanDuration` (asserts the 59.6s / 1m59.6s / 59m59.6s carry cases) |
| 18 | **A live keymap rebind stayed dead until reload even with the attribute swapped** — OOB-swapping `<body data-keymap>` alone did not relive the dispatcher. Caught while building V10 (keybinding live-apply). | The vanilla-JS dispatcher built its `key→action` reverse map **once at load** and read that cached map on every keydown, never re-reading `<body data-keymap>`. So a Settings POST that only updated the attribute (the obvious "OOB-swap the attribute" approach) would leave the new binding inert until a full reload — the live attribute and the dispatcher's map would silently desync. | The Settings keybinding POST appends an `applyKeymap(...)` script that updates `document.body.dataset.keymap` **and** rebuilds the dispatcher's reverse map from one source; load-time init routes through the same `applyKeymap`, so the attribute and the dispatcher can't drift. The emitted keymap is read back from the **persisted** config, so a no-op/rolled-back save re-emits the in-sync keymap (or, on the error branch, nothing) and never desyncs live from disk. | unit: `internal/web` `TestSettingsSaveLiveAppliesKeymapViaOOB`, `TestSettingsSaveLiveApplyMatchesPersistedKeymap`, `TestSettingsSaveLiveApplyEscapesBinding`; browser: `e2e/tests/keybindings.spec.ts` "a rebind takes effect live, without a full page reload" |
| 19 | **An open ⌘K command palette didn't suppress other keyboard shortcuts** — pressing the help key (`?`) while the palette was open stacked the help overlay on top of it (two `aria-modal` dialogs). Caught by the pre-merge code review of the V22 diff. | The modal guard was `(helpOpen \|\| cmdkOpen) && action !== 'help'`: the `!== 'help'` carve-out (so the help key can *close* the help overlay) also let `?` through while the **palette** was open, since the guard didn't distinguish which overlay was up. With focus off the filter input (e.g. after a backdrop click), `?` then opened a second modal behind the palette. | Split the guard: the palette blocks **every** bound key (`if (overlayOpen('cmdk-overlay')) return;` — it has no toggle key, ⌘K/Esc are handled earlier), and only the help overlay keeps the `action !== 'help'` carve-out so its own key still closes it. | browser: `e2e/tests/palette.spec.ts` "an open palette suppresses other shortcuts (no second modal)" |
| 20 | The schema-driven **elicitation form's secondary text failed WCAG AA contrast on the light theme** (`.elicit-desc` ~4.13:1). Surfaced by the V23 diff: the both-theme axe scan caught it once the demo's completed elicit form sat in the shared chat timeline during the (full-suite) command-palette scan. | `.elicit-desc` / `.elicit-source` dimmed via `opacity` on the inherited `--fg`, which composites the text toward the bluish form fill (`rgba(110,168,254,.10)` over `--bg`) and drops below 4.5:1 on the light theme — `opacity` dims foreground *and* the contrast with the background. | Dim with the AA-tuned `--dim` token (no `opacity`), which clears 4.5:1 on the form fill in both themes. The chat post-turn a11y scan now waits for `#elicits .elicit-desc` so the form's contrast is checked in both themes every run (a deterministic guard, not the palette test's incidental catch). | browser: `e2e/tests/a11y.spec.ts` "chat page is accessible after a streamed turn — light/dark" (now waits for the elicit form) |
...
| 21 | The **spend-source badge** (`reported`/`est`/`mixed`, ADR-0033) **failed WCAG AA contrast** on the light theme — `.cost-src.est` at ~4.44:1. The badge sits in the statusline and cost footer on **every** page, so the both-theme axe scan went red across the whole static-page suite. Surfaced when e2e first ran against the 0057 diff: the `-race` unit gates were green, but Playwright browsers weren't available in the build sandbox, so the regression wasn't caught before push. | `.cost-src` was dimmed with `opacity: .85`, which composites the `--dim` token (`#5b6066`) over the white background to `#74787d` and drops the `est` badge to 4.44:1, just under 4.5 — `opacity` dims the foreground *and* its contrast (same failure mode as #20 / #10). | Drop the `opacity`; the `--dim`/`--accent` tokens already clear 4.5:1 unmuted (`--dim` on white ≈ 6.35:1). | browser: `e2e/tests/a11y.spec.ts` (axe-core, no A/AA violations — the badge renders in the statusline/footer on every static page in both themes) |
| 22 | The markdown renderer could **leak raw NUL placeholder sentinels into output** — `` `0`1`x` `` (a bare digit between two inline-code spans) rendered `\x000…\x00`-bearing HTML. Latent since ADR-0001; surfaced by `FuzzRenderMarkdown` while refactoring to the block-AST seam (0077), reproduced on the pre-refactor `origin/main`. | `inline()` restored stashed fragments with descending `strings.ReplaceAll` of `\x00idx\x00`; with placeholders adjacent around a literal digit, ReplaceAll matched a **fake placeholder spanning two real ones** (close-NUL + digit + open-NUL), consuming their sentinels and stranding the real ones. | Restore with a single left-to-right scan pairing NULs (every NUL is structural — input NULs are stripped first), recursing into stashed fragments since a link's stash can embed a code-span placeholder. Fake cross-boundary matches are impossible by construction. | unit: `internal/web` fuzz seed corpus `testdata/fuzz/FuzzRenderMarkdown/{2986563d…,0a4f05d7…}` (run by `make test`); `FuzzRenderMarkdown` NUL-leak assertion |
| 23 | **`make doctor` couldn't find the recipe catalog** — failed `bash: …/skills/recipe-doctor/scripts/run-doctors.sh: No such file or directory` (exit 127) in every plain shell / web session, even with the `cookbook@ori` plugin installed and enabled. Cost multiple debugging sessions. | The Makefile resolved the catalog as `RECIPES_DIR ?= $(CLAUDE_PLUGIN_ROOT)` with a `../claude-skills/plugins/cookbook` sibling-checkout fallback (ADR-0054). Both are empty in practice: **`$CLAUDE_PLUGIN_ROOT` is set only inside a plugin's own command/hook/skill execution, never exported to the session shell or a user-run `make`** (verified unset even in a login shell); and there is no sibling checkout. The doctors only ran when a path was supplied by hand. | Add the **marketplace install location** as the resolving fallback — `$(firstword $(wildcard $(HOME)/.claude/plugins/marketplaces/*/plugins/cookbook) ../claude-skills/plugins/cookbook)`. That dir is the stable path recorded in `known_marketplaces.json` (`installLocation`), present whenever the marketplace is installed, and marketplace-name-agnostic. `RECIPES_DIR=` still overrides for CI. | advisory: `make doctor` returns 0 (all recipe doctors green) in any session where the cookbook plugin is installed — not a CI gate (ADR-0054: CI has no plugin/catalog checkout), so no automated test guards it |
...
| A document-level `htmx:afterSwap` listener to react to an SSE-swapped marker (S6 attention dot) | The handler never fired for the SSE `attention` swap — `e.detail.target` didn't match the `#attention` id under the htmx-ext-sse swap path, so the title/favicon never updated (the e2e `toHaveTitle` stayed `my-orchestra`). Depending on the afterSwap event's target semantics for SSE swaps is brittle. | Carry an inline `<script>` in the swapped fragment itself (`attentionMarker` → `<i data-count=N><script>applyAttention(N)</script>`) — htmx runs scripts in swapped content, so the reaction is co-located with the data. The **same live-apply idiom the keymap OOB swap uses** (`help.go` → `<script>applyKeymap(…)</script>`). Apply once more from the server-rendered initial count on load. |
