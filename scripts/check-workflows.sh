...
# silently, so they can't land. Deterministic; wired into CI (the lint job) AND
# the local lint gate (`make lint`), and runnable as a Claude Code Stop hook
# (.claude/settings.json), so a violation is caught before merge and before a
# session ends. Exits non-zero with a clear message on any violation.
# — see docs/CONVENTIONS.md "Quality gates"
#
# Invariants enforced (portable to any repo with .github/workflows):
...
if [ ! -d .github/workflows ]; then
  echo "no .github/workflows — nothing to check"
  exit 0
fi
...
if ! DEFAULT_BRANCH="$branch" python3 - <<'PY'; then
...
echo "workflow checks passed (default branch: $branch)"
